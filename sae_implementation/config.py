...
class CacheActivationsRunnerConfig:
    ...
    train_batch_size_tokens: int = 0 # It is not used for caching, we define it for generality with LanguageModelSAERunnerConfig 
    ...
    # Presence or not of an end-of-sentence token, it enables us to know where to locate the token containing the class prediction
    eos : bool = False
    # In order to know if we have to shape the forward pass so that it handles the added prompt embeddings (implementation of this feature is not completed)
    prompt_tuning : bool = False

    # Whether we also cache the labels, in that case the labels are concatenated to the activations vector
...
class LanguageModelSAERunnerConfig:
    ...
    Args:
        ...
        hook_name (str): Name of the hook from which the activations we train on are extracted. This should be a valid TransformerLens hook.
        hook_layer (int): The index of the layer to hook. Used to stop forward passes early and speed up processing.
        
        use_cached_activations (bool): Whether to use cached activations.
        cached_activations_path (str, optional): The path to the cached activations.
        d_in (int): The input dimension of the SAE (dimension of the extracted hidden states from the LLM)
        ...
        scale_sparsity_penalty_by_decoder_norm (bool): Whether to scale the sparsity penalty (on the input, not the feature sparsity) by the decoder norm.
        ...
        num_classifier_features (int) : number of features within the SAE hidden layer used as inputs for the trained imitation classifier (only useful if lmbda_classifier > 0 ). It defines the dimension of z_class.
        nb_classes (int) : number of classes among which the LLM classifier must select its decision for the sentence (only useful if lmbda_classifier > 0 )
        feature_activation_rate (float) : Targeted maximum fraction of sentences on which a feature is activen (i.e non zero).Feature sparsity loss becomes non-zero above this threshold.
    ...
    nb_classes: int = 4
    feature_activation_rate: float = 0.0
    ...
    def __post_init__(self):
        ...
        if self.verbose:
            ...
            print(f"n tokens per loaded buffer : {n_tokens_per_buffer}")
            n_contexts_per_buffer = (
                self.store_batch_size_prompts * self.n_batches_in_buffer
            )
            print(
                f"n sentences per loaded buffer : {n_contexts_per_buffer}"
            )

            total_training_steps = self.training_tokens // self.train_batch_size_tokens
    ...
    # We just consider training_steps = total_training_steps as we do not fine-tune the SAE in our implementation
    @property
