"""Most of this is just copied over from jbloomAus/SAELens Github and slightly simplified:
...
from .toolkit.pretrained_saes_directory import (
    get_norm_scaling_factor,
    get_pretrained_saes_directory,
)

from .classifier import MLPClassifier, MLPClassifierConfig
...
class SAE(HookedRootModule):
    ...
    def initialize_weights_basic(self):
        ...
        # classifier_parameters = { 'input_dim' : self.cfg.classifier_input_dim, 'hidden_dim' : self.cfg.classifier_hidden_dim, 'nb_hidden_layer' : self.cfg.classifier_nb_hidden_layer,'output_dim' : self.cfg.classifier_output_dim, 'dropout' : self.cfg.classifier_dropout }
        
        # self.classifier = MLPClassifier(
        #     MLPClassifierConfig(**classifier_parameters)
        # )
        ...
        self.classifier_weight = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(
                    4,self.cfg.num_classifier_features, dtype=self.dtype, device=self.device
                )
            )
        )
        
        # Optionally define a bias term
        self.classifier_bias = nn.Parameter(
            torch.zeros(4, dtype=self.dtype, device=self.device)
        )

        # # Mixture of Gaussian parameters
        # self.pi_logits = nn.Parameter(torch.zeros(8,device=self.device))  # Unbounded logits
        # self.mu = nn.Parameter(torch.randn(8, self.cfg.d_sae,device=self.device))       # Spread means
        # #self.sigma = nn.Parameter(torch.ones(8,device=self.device) * 1)         # Set initial variances
    ...
    def forward(
        ...
        if self.use_error_term and self.cfg.architecture != "gated":
            with torch.no_grad():
                ...
                x_reconstruct_clean = (feature_acts) @ self.W_dec + self.b_dec
        ...
        # TODO: Add tests
            ...
            with torch.no_grad():
                ...
                x_reconstruct_clean = feature_acts_clean @ self.W_dec + self.b_dec
