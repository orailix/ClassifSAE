...
class ActivationsStore:
    ...
    def __init__(
        ...
        self.model = model
        #print("Padding side of the tokenizer : ",self.model.tokenizer.padding_side)
        ...
        self.context_size = 1 #We only take one token i.e. the one preceding the predicting token
        ...
        #We use the classification approach of only keeping the activation of the token preceding the one that predicts the class
    ...
    def get_buffer(
        ...
        """
        context_size = self.context_size #We only take one token i.e. the one preceding the predicting token
        ...
        if self.cached_activations_path is not None:
            ...
            # Initialize an empty tensor with an additional dimension for layers (we also add one dimension if we have to stack the labels)
        ...
        if self.save_label:
            buffer_labels = torch.zeros((total_size),dtype=self.dtype,device=self.device)


        print(f"self.dataset 2 : {self.dataset}")
        '''We verify that the remaining prompts of the dataset are enough to match total_size 
        (in case we arrive at the end of the dataset) because we want all our buffers to be of 
        same size.'''
        remaining_prompts = len(self.dataset) - self.dataset_idx_last_token
        if remaining_prompts > total_size :
            selected_dataset = self.dataset.select(
                list(range(self.dataset_idx_last_token, self.dataset_idx_last_token + total_size))
            )
            # selected_dataset = Dataset.from_dict(self.dataset[self.dataset_idx_last_token:(self.dataset_idx_last_token+total_size)])
            self.dataset_idx_last_token += total_size
        else: 
            '''
            In that case, we know it only concerns the last buffer since
            n_buffers = math.ceil(self.cfg.training_tokens / total_size): defined in cache_activations_runner .run
            '''
            ...
            # Calculate the number of additional elements needed to complete the sample
            n_elements_to_complete = total_size - remaining_prompts
            ...
            # and then keeping only the tokens column
            completion_dataset = self.dataset.select(rand_indices)

            # end_of_dataset = Dataset.from_dict(self.dataset[self.dataset_idx_last_token:])
            # n_elements_to_complete = total_size - remaining_prompts
            # rand_indices = np.random.choice(len(self.dataset),n_elements_to_complete,replace=True)
            # completion_dataset = self.dataset.select(rand_indices).select_columns([self.tokens_column])
        ...
        with torch.no_grad():
            ...
            for num_batch, batch in enumerate(dataloader_sentence_hidden_states):
                ...
                # true_labels = batch['token_labels'][:,(-1-int(self.is_eos))].to(self.model.cfg.device)
                predicted_labels = batch['predicted_labels'].to(self.model.cfg.device)
               
                
                #If our model is a PromptTunerForHookedTransformer, the forward pass expect additionnal tokens at the begginning of the sentence
                if self.prompt_tuning:
                    batch_size = batch_tokens.size(0)
                    prompt_token_ids = torch.full(
                        (batch_size, self.model.num_prompt_tokens),
                        self.model.tokenizer.eos_token_id,  # Using EOS token as a placeholder
                        dtype=torch.long,
                        device=batch_tokens.device,
                    )
                    batch_tokens = torch.cat([prompt_token_ids, batch_tokens], dim=1)
                ...
                with autocast_if_enabled:
                    ...
                    transformer-lens and it causes conflict with the positional embedding that it dones on a vector of size the context attention windows'''
                    if batch_tokens.shape[1] > self.model.cfg.n_ctx:
                        attention_mask = attention_mask[:,-self.model.cfg.n_ctx:]
                        batch_tokens = batch_tokens[:,-self.model.cfg.n_ctx:] #It should not be an issue since we want to predict for almost the last token, so it looses almost nothing of the context it could have seen otherwise
                ...
                n_prompts, max_length = batch_tokens.shape
        
                stacked_activations = torch.zeros((n_prompts, context_size, 1, self.d_in))
                
            
                #WARNING : It assumes that the activation does not come from a head dimension !
                #We only select the activation of the hidden state above the token preceding the token to predict to do classification
                stacked_activations[:, :, 0] = layerwise_activations[self.hook_name][:,(-2-int(self.is_eos)),:].unsqueeze(1)
                ...
                #If we save the labels (for the classifier head)
                if self.save_label:
        ...
        if self.save_label:
            ...
            #We match the token labels to categorical one
            vocab = self.model.tokenizer.get_vocab()
            unique_labels = np.unique(buffer_labels.numpy())
            keys_labels = set(unique_labels)
            

            match_token_label_class = { value : int(key) for key,value in vocab.items() if value in keys_labels }
            ...
            #print(f"buffer_labels : {buffer_labels}")
            buffer_labels = lookup_table[buffer_labels]
            #print(f"buffer_labels : {buffer_labels}")
