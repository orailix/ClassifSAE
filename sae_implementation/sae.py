...
class SAE(HookedRootModule):
    ...
    def initialize_weights_basic(self):
        ...
        # Define the learnable weight matrix for the activations linear classifier
        self.classifier_weight = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(
                    self.cfg.nb_classes,self.cfg.num_classifier_features, dtype=self.dtype, device=self.device
                )
            )
        )
        
        # Define a bias term for the activations linear classifier
