...
class EvaluationConfig:
    """
    Configuration for evaluating concepts extracted by the different evaluated methods
    ...
    Args:
        ...
        split (str) : either train or test, wether the method was trained on "train" or "test"
        ...
        causality (bool) : Wether we run an ablation study on the selected concepts after the simple accuracy reconstruction evaluation
    ...
    def __init__(self,
        ...
        self.metrics_reconstruction = os.path.join(PATH_INTERP_METHODS_METRICS)
        self.activations_interpretability_methods = os.path.join(PATH_INTERP_METHODS_ACTIVATIONS)
        ...
        if method_name in ["ica","concept_shap"]:
            ...
            assert os.path.exists(os.path.join(PATH_DR_METHODS, split)), f"Error: The folder {os.path.join(PATH_DR_METHODS, split)} does not exist"
        ...
        #Path to retrieve activations from the dataset with the ground truth labels (ActivationDataset)
        self.dir_acts_with_labels = PATH_ACTS_LABELS 

        #Path to retrieve and save the features selected and segmented in the post-processing
        self.post_processed_features_path = PATH_POST_FEATURES

        #Wether we use our heuristic in post-processing to only keep features participating the most in the recovering accuracy
        self.features_selection = True
